// TO RUN THIS FIRST COMPILE THE CODE USING g++ pathfinder.cpp -o routecli.exe
//THEN RUN THIS USING npm start
...
pair<double,double> getCoordinates(string place) {
   string command =
"curl -s \"https://nominatim.openstreetmap.org/search?format=json&q="
+ place +
"\" > temp.json";
...
void buildGraph() {
    ...
    for(int i=0;i<n;i++) {
        ...
        for(int k = 0; k < 10 && k < (int)distList.size(); k++) {
            int neighbor = distList[k].second;
            double dist = distList[k].first;
            double cost = calculateCost(dist);
            adj[i].push_back({neighbor, dist, cost});
            adj[neighbor].push_back({i, dist, cost});
        }
    }
}

double calculatePathDistance(vector<int> &path)
{
    double total = 0;
...
void dfs(string source, string dest)
{
    ...
    if(pathfind(start, end, visited, path))
    {
        ...
        cout<< "Estimated Cost: Rs. "<<calculateCost(totalDistance)<< endl;
    }
    else
    {
...
void dijkstra(string source, string dest)
{
    ...
    while(!pq.empty())
    {
        ...
        {
            int adjvertex = adjnode.neighbor;
            double adjwgt = adjnode.distance;
            double d = dis + adjwgt;
...
// ---------- JSON helpers for web API ----------
void jsonEscape(ostream &out, const string &s)
{
    out << '"';
    for (char c : s) {
        if (c == '"' || c == '\\') out << '\\' << c;
...
bool dijkstraPath(string source, string dest, vector<int> &outPath, double &outDist)
{
    if (cityIndex.find(source) == cityIndex.end() ||
        cityIndex.find(dest) == cityIndex.end())
        return false;
    ...
    if (distance[end] == 1e9) return false;

    outPath.clear();
    for (int v = end; v != -1; v = parent[v])
        outPath.push_back(v);
    reverse(outPath.begin(), outPath.end());
    outDist = calculatePathDistance(outPath);
    return true;
}

bool bfsPath(string source, string dest, vector<int> &outPath, double &outDist)
{
    if (cityIndex.find(source) == cityIndex.end() ||
        cityIndex.find(dest) == cityIndex.end())
        return false;
...
bool dfsPath(string source, string dest, vector<int> &outPath, double &outDist)
{
    if (cityIndex.find(source) == cityIndex.end() ||
        cityIndex.find(dest) == cityIndex.end())
        return false;
    ...
    function<bool(int)> dfsVisit = [&](int node) -> bool {
        visited[node] = 1;
        outPath.push_back(node);
        if (node == end) return true;
        for (auto neigh : adj[node]) {
            int next = neigh.neighbor;
            if (!visited[next] && dfsVisit(next)) return true;
        }
        outPath.pop_back();
        return false;
    };
...
void printPathJson(const string &algo, const vector<int> &path, double dist)
{
    cout << "{\"success\":true,\"algorithm\":\"" << algo << "\",\"path\":[";
    for (size_t i = 0; i < path.size(); i++) {
        if (i) cout << ',';
        jsonEscape(cout, nodes[path[i]].name);
    }
    cout << "],\"pathCoords\":[";
    for (size_t i = 0; i < path.size(); i++) {
        if (i) cout << ',';
        cout << '[' << nodes[path[i]].lat << ',' << nodes[path[i]].lon << ']';
    }
    cout << "],\"distance\":" << dist
         << ",\"cost\":" << calculateCost(dist) << '}';
}

int cliSignup(const string &email, const string &pass)
{
    ofstream file("users.txt", ios::app);
    file << email << '|' << pass << endl;
    file.close();
    cout << "{\"success\":true,\"message\":\"Account created\"}";
    return 0;
}

int cliLogin(const string &email, const string &pass)
{
    ifstream file("users.txt");
    user u;
    while (getline(file, u.email, '|') && getline(file, u.password)) {
        if (email == u.email && pass == u.password) {
            file.close();
            cout << "{\"success\":true,\"email\":";
            jsonEscape(cout, email);
            cout << '}';
            return 0;
        }
    }
...
int cliGeocode(const string &src, const string &dest)
{
    auto s = getCoordinates(src);
    auto d = getCoordinates(dest);

    if ((s.first == 0.0 && s.second == 0.0) || (d.first == 0.0 && d.second == 0.0)) {
        cout << "{\"success\":false,\"message\":\"Could not geocode location\"}";
        return 1;
    }

    addLocation(src, dest);

    ofstream out("coords.json");
    out << fixed;
    out << "{\"success\":true,\"src\":[" << s.first << ',' << s.second
        << "],\"dest\":[" << d.first << ',' << d.second << "]}";
    out.close();

    cout << "{\"success\":true,\"src\":[" << s.first << ',' << s.second
         << "],\"dest\":[" << d.first << ',' << d.second << "]}";
    return 0;
}

int cliFindPath(const string &algo, const string &src, const string &dest)
{
    string source = toLowerString(src);
    string destination = toLowerString(dest);

    addLocation(source, destination);
    loadData();
    buildGraph();
    ...
    double dist = 0;
    bool ok = false;
    string algoLower = toLowerString(algo);

    if (algoLower == "dfs") ok = dfsPath(source, destination, path, dist);
    else if (algoLower == "bfs") ok = bfsPath(source, destination, path, dist);
    else ok = dijkstraPath(source, destination, path, dist);
    ...
    string algoName = (algoLower == "dfs") ? "DFS" : (algoLower == "bfs") ? "BFS" : "Dijkstra";
    printPathJson(algoName, path, dist);
    return 0;
}

int cliAddTrip(const string &email, const string &src, const string &dest,
               const string &date, double dist, double cost)
{
    ofstream file("trips.txt", ios::app);
    file << email << '|' << src << '|' << dest << '|' << date << '|'
         << dist << '|' << cost << endl;
    file.close();
    cout << "{\"success\":true,\"message\":\"Trip saved\"}";
    return 0;
}

int cliListTrips(const string &email)
{
    ifstream file("trips.txt");
    trip t;
    cout << "{\"success\":true,\"trips\":[";
    bool first = true;
    ...
    while (getline(file, t.email, '|') &&
           ...
           file >> t.cost &&
           file.ignore()) {
        if (t.email != email) continue;
        if (!first) cout << ',';
        first = false;
        cout << "{\"source\":";
        jsonEscape(cout, t.source);
        cout << ",\"destination\":";
        jsonEscape(cout, t.destination);
        cout << ",\"date\":";
        jsonEscape(cout, t.date);
        cout << ",\"distance\":" << t.distance << ",\"cost\":" << t.cost << '}';
    }
    file.close();
    cout << "]}";
    return 0;
...
{
    if (argc < 2) {
        cout << "{\"success\":false,\"message\":\"Usage: route_cli <command> [args]\"}";
        return 1;
    }

    string cmd = toLowerString(argv[1]);

    if (cmd == "signup" && argc >= 4)
        return cliSignup(argv[2], argv[3]);
    if (cmd == "login" && argc >= 4)
        return cliLogin(argv[2], argv[3]);
    if (cmd == "geocode" && argc >= 4)
        return cliGeocode(argv[2], argv[3]);
    if (cmd == "path" && argc >= 5)
        return cliFindPath(argv[2], argv[3], argv[4]);
    if (cmd == "addtrip" && argc >= 8)
        return cliAddTrip(argv[2], argv[3], argv[4], argv[5], stod(argv[6]), stod(argv[7]));
    if (cmd == "listtrips" && argc >= 3)
        return cliListTrips(argv[2]);

    cout << "{\"success\":false,\"message\":\"Unknown command\"}";
    return 1;
}
